...
test('силовой объём по группам совпадает с разметкой', () => {
  ...
  // Хват резинки (Task 11): из семи размеченных упражнений «Разведение
  // резинки над головой» разминочное и в силовой подсчёт не входит. Остаются
  // шесть силовых, у каждого 0.5 доли и 2 круга блока: 0.5 × 2 × 6 = 6,
  // вровень с ягодичными.
  assert.equal(s.get('forearms'), 6);
});
...
test('разминка считается отдельно и даёт дробные значения', () => {
  ...
  // Хват резинки (Task 11): «Разведение резинки над головой» — единственное
  // разминочное упражнение с предплечьями, 0.5 × 3 круга.
  assert.equal(w.get('forearms'), 1.5);
...
test('заминка считается отдельно', () => {
  ...
  // «Спокойное дыхание» — единственное упражнение заминки с прессом в разметке,
  // и разметку у него убрали: дыхание не тренирует ничего. Пустой load обязан
  // просто не дать слагаемого, а не ноль и не NaN.
  assert.equal(c.get('abs'), undefined);
});

// Число из брифа Task 8, посчитанное по данным: 1 («Подкручивание таза лёжа»,
// 1 круг) + 1.5 (подъём колена, 0.5 × 3) + 3 (подъёмы ног, 1 × 3) из разминки
// и 1 («Птица-собака», 0.5 × 2) из силовых. Заминка в пресс больше не приходит,
// поэтому 6.5, а не 7.
test('пресс во всей нагрузке даёт 6,5 подхода, и дыхания в списке упражнений нет', () => {
  const abs = regionSummary(legs, ['warmup', 'strength', 'cooldown']).get('abs');
  assert.equal(abs.sets, 6.5);
  assert.deepEqual(abs.exercises, [
    'Подкручивание таза лёжа',
    'Подъём колена с лёгкой резинкой',
    'Подъём и опускание вытянутых ног лёжа',
    '«Птица-собака»: противоположные рука и нога',
  ]);
  // Силовые у пресса правка не задела: «Птица-собака» осталась как была.
  assert.equal(regionSummary(legs, ['strength']).get('abs').sets, 1);
});
...
// расходилось у половины регионов — у восьми из шестнадцати, например пресс
// 7 вместо 6,5 и косые 5 вместо 4.
