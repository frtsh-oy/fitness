...
test('подписи про разминку на месте, а фразы про неразмеченные группы у legs-mwf больше нет', () => {
  const { window } = mount();
  window.document.querySelector('.bodymap-toggle').click();
  const groups = window.document.querySelector('.bodymap-groups').textContent;

  // Список здесь дословный: это разметка legs-mwf, и тест сторожит именно
  // её — состав, порядок и строчные буквы внутри фразы (так в согласованном
  // макете). А что список СЧИТАЕТСЯ, а не вписан в текст, проверяет тест с
  // фикстурой ниже: сравнением с реальной тренировкой этого не отличить,
  // потому что вписанный литерал и производное значение здесь совпадают.
  assert.match(groups, /Только в разминке и заминке: сгибатели бедра, задняя дельта\./);

  // Task 11: хват резинки размечен предплечьям в семи упражнениях —
  // idleGroups(legs) опустел, и второй фразы для этой тренировки больше нет
  // вовсе, а не с другим списком. Раньше здесь стояла проверка «Светлым — то,
  // чего нет в разметке: предплечья.»; ветка app.js, которая склеивает фразы
  // через пробел (parts.join(' ')), для legs-mwf с этой правки недостижима —
  // список parts длины 1, join не добавляет ничего лишнего. Синтетическая
  // тренировка ниже («списки групп в подписи…») по-прежнему даёт обе фразы
  // сразу и держит проверку самой склейки.
  assert.equal(groups, 'Только в разминке и заминке: сгибатели бедра, задняя дельта.',
    'при пустом idleGroups строка не должна содержать фразу «Светлым» или обрывок после неё');

  // Подпись говорит про разметку, а не про тело — формулировку уже ломали
  // один раз, проверка остаётся на будущее.
...
test('списки групп в подписи — из разметки открытой тренировки, а не вписаны в текст', (t) => {
  ...
  // Task 11: у настоящей тренировки idleGroups опустел — хват резинки
  // размечен предплечьям. Фикстура выше это не наследует: notesFixture
  // подменяет load каждого упражнения целиком (см. её код), поэтому её
  // собственные calves/forearms остаются незадействованными независимо от
  // того, что размечено в legs-mwf.js.
  assert.deepEqual(idleGroups(legs), []);
  ...
  // отдельно и склейку вплотную пропускают: мутация parts.join('') давала бы
  // на экране «…бицепс.Светлым — то, чего нет…» и не роняла бы ни одну из
  // них. У legs-mwf для этой ветки после Task 11 нет данных (там теперь одна
  // фраза или ни одной), поэтому единственная тренировка, которая всё ещё
  // даёт обе фразы сразу и держит эту проверку, — фикстура здесь.
  assert.match(groups, /бицепс\. Светлым/,
...
test('клик по другой мышце заменяет прежний список, а не дополняет его', () => {
  ...
  // Задняя дельта — единственная группа без силового подхода в этом режиме
  // по умолчанию (с Task 11 предплечья размечены и этому определению больше
  // не отвечают): её единственное упражнение, «Разведение резинки над
  // головой», разминочное.
  const title = pickByTitle(d, window, 'Задняя дельта');
  assert.equal(title, 'Задняя дельта — 0 подх.');
...
test('строка про разминку и заминку есть только в силовом режиме; у legs-mwf неразмеченных групп нет ни в одном режиме', () => {
  const { window } = mount();
  const d = window.document;
  d.querySelector('.bodymap-toggle').click();
  const groups = () => d.querySelector('.bodymap-groups').textContent;
  ...
  assert.match(groups(), /Только в разминке и заминке: сгибатели бедра, задняя дельта/);
  // Task 11: хват резинки размечен предплечьям в семи упражнениях —
  // idleGroups(legs) не зависит от режима карты (bodymap.js) и теперь пуст
  // в обоих. Раньше здесь стояла проверка на фразу «чего нет в разметке:
  // предплечья» и в силовом режиме тоже; теперь фразы «Светлым» для этой
  // тренировки нет вовсе, ни в одном режиме.
  assert.doesNotMatch(groups(), /Светлым/);

  modeButton(d, 'all').click();
  // В этом режиме сгибатели бедра и задняя дельта раскрашены наравне с
  // силовыми: бледность объяснять больше нечем, и первая строка уходит
  // совсем, а не переписывается. Неразмеченных групп по-прежнему нет —
  // вторая строка тоже отсутствует, и под картой не остаётся текста вовсе:
  // это и есть ветка «list пуст» из app.js, раньше проверенная только на
  // синтетической тренировке (см. «у тренировки, где нечего оговаривать…»
  // выше), а теперь ставшая реальным поведением legs-mwf.
  assert.doesNotMatch(groups(), /разминк/i,
    'в режиме всей нагрузки разминка и заминка уже в цвете — говорить о них нечего');
  assert.doesNotMatch(groups(), /сгибатели бедра|задняя дельта/i);
  assert.equal(groups(), '', 'в режиме всей нагрузки под картой у legs-mwf не должно остаться текста');
