...
test('подпись объясняет, что самый тёмный оттенок — это верх шкалы', () => {
  // Шкала из шести оттенков, а в режиме всей нагрузки объёмы доходят до 12.5:
  // всё, что от шести и выше, красится одним цветом. Знать об этом человеку
  // неоткуда, кроме подписи, поэтому она говорит это прямо — в обоих режимах,
  // шкала-то одна.
  const { window } = mount();
  const d = window.document;
  d.querySelector('.bodymap-toggle').click();
  const note = () => d.querySelector('.bodymap-note').textContent;
  assert.match(note(), /Самый тёмный оттенок — 6 подходов и больше/);
  modeButton(d, 'all').click();
  assert.match(note(), /Самый тёмный оттенок — 6 подходов и больше/);
});
