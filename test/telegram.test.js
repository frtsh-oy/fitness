...
test('WebApp есть, но пустой объект без единого метода — слой доступен и ни один вызов не падает', async () => {
  const webApp = {};
  ...
  // Подтверждение — тоже вызов слоя, и на пустом объекте оно обязано не зависнуть,
  // а деградировать: спросить нечем, значит действие человека выполняется.
  assert.equal(await tg.confirm('Точно?'), true);
});
