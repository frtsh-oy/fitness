...
//
// Отдаём и цвет, снятый ДО клика. Клик выбирает область, а карта сразу заливает
// выбранную синим и заменяет все полигоны новыми, — значит по найденному узлу
// цвет невыбранной области уже не прочитать: он отсоединён от документа и
// остался с прежним style.fill. Прежний он именно потому, что querySelectorAll
// отдаёт снимок: все полигоны перебора — из той отрисовки, которая была на
// экране до первого клика по этому виду. Цвет выбранной области проверяется
// отдельно, по живой разметке (blueCount ниже).
function findByRegion(hosts, picks, region) {
  for (const host of hosts) {
    ...
    for (const polygon of host.querySelectorAll('polygon')) {
      ...
      if (picks[0]?.region === region) return { polygon, fill };
    }
  }
  return null;
}
