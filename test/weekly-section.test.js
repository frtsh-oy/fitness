...
// Раунд правок 1, третий ответ координатора: теперь оговорок четыре
// (количество/качество, эксцентрическая нагрузка резинок, счёт односторонних,
// потолок «рабочего объёма»), и каждая должна быть удержана СВОИМ
// утверждением — общая проверка «где-то в секции есть такие слова» не
// отличает удаление одной оговорки от другой, если слова совпадают, и
// именно на этом раньше попался тест на пометку «наша линия» (находка 2).
// Поэтому четыре отдельных теста, а не один общий, и каждый ищет внутри
// .weekly-note — не по всей секции, где ещё лежат пилюли.
...
test('секция говорит: резинки дают меньше эксцентрической нагрузки, чем свободные веса', () => {
  const { window, document } = mount();
  startApp(window);
  document.querySelector('.weekly-toggle').click();
  assert.match(noteText(document), /эксцентрической нагрузки/,
    'нет оговорки про меньшую эксцентрическую нагрузку резинок');
});
