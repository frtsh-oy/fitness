...
// Раунд правок 1, третий ответ координатора: оговорок на экране пять (что
// значит пометка «наша линия», количество/качество, чем резинка отличается от
// свободного веса, счёт односторонних, потолок «рабочего объёма»), и каждая
// должна быть удержана СВОИМ утверждением — общая проверка «где-то в секции
// есть такие слова» не отличает удаление одной оговорки от другой, если слова
// совпадают, и именно на этом раньше попался тест на пометку «наша линия»
// (находка 2). Поэтому пять отдельных тестов, а не один общий, и каждый ищет
// внутри .weekly-note — не по всей секции, где ещё лежат пилюли.
function noteText(document) {
  return [...document.querySelectorAll('.weekly-note')].map(n => n.textContent).join(' ');
}

test('секция говорит: считает количество, а не качество', () => {
  const { window, document } = mount();
  startApp(window);
  document.querySelector('.weekly-toggle').click();
  assert.match(noteText(document), /количество, а не качество/,
    'нет оговорки про то, что усилие из данных не видно');
});

// Спека, раздел «Что калькулятор обязан говорить о себе», ограничение 2.
// В первой сдаче Task 3 на экране была только оговорка про количество вместо
// качества (ограничение 1) — эта отсутствовала нигде в продукте, только в
// самой спеке. Ревью подняло вопрос, координатор подтвердила: спека относит
// ограничение к разделу про то, что калькулятор обязан говорить о СЕБЕ, а не
// к описанию решения, — значит место ему на экране, а не только в спеке.
//
// Финальное ревью (К1): раньше оговорка утверждала «меньше эксцентрической
// нагрузки… рост чуть уступает» — и ссылка под этим утверждением его не
// подтверждала. Утверждение снято, осталось то, что верно без источника.
// Тест ищет ровно это, а не прежние слова: проверять «сказано хоть что-то про
// резинки» значило бы пропустить возврат неподтверждённого утверждения.
test('секция говорит: резинка нагружает иначе, а поправки под неё у нас нет', () => {
  const { window, document } = mount();
  startApp(window);
  document.querySelector('.weekly-toggle').click();
  assert.match(noteText(document), /натяжение меняется по ходу движения/,
    'нет оговорки про то, чем резинка отличается от свободного веса');
  assert.match(noteText(document), /поправки на резинку в проверенных источниках нет/,
    'оговорка не говорит, что числа под резинку не скорректированы');
});

test('секция говорит: односторонние считаются по сторонам — это наше чтение', () => {
  const { window, document } = mount();
  startApp(window);
  document.querySelector('.weekly-toggle').click();
  assert.match(noteText(document), /наше чтение/,
...
test('секция объясняет, что значит пометка «наша линия», а не только показывает её в пилюлях', () => {
  ...
  startApp(window);
  document.querySelector('.weekly-toggle').click();
  assert.match(noteText(document), /означает: вся его граница — наше решение, а не источник/,
...
test('секция объясняет, что верхняя граница «рабочего объёма» — наша линия, а нижняя подтверждена источником', () => {
  ...
  startApp(window);
  document.querySelector('.weekly-toggle').click();
  assert.match(noteText(document), /нижняя граница \(десять подходов\) подтверждена источником/,
    'нет оговорки про то, что у «рабочего объёма» источник подтверждает только вход, а не потолок');
});

// Шапка списка — отдельный абзац (.weekly-head), и до финального ревью её не
// держал ни один тест (М12): все пять .weekly-note покрыты своим тестом, а
// единственный текст, объясняющий, какое из двух чисел какое, удалялся с
// зелёными тестами. Ищется именно в .weekly-head, а не в noteText: иначе
// проверка пережила бы переезд этого объяснения в любую из оговорок, а его
// место — над списком, до чисел, а не после двадцати строк.
test('над списком сказано, какое из двух чисел какое', () => {
  const { window, document } = mount();
  startApp(window);
  document.querySelector('.weekly-toggle').click();
  const head = document.querySelector('.weekly-head');
  assert.ok(head, 'нет абзаца .weekly-head над списком');
  assert.match(head.textContent, /Первое число — целевая работа, по нему и вердикт/,
    'шапка не говорит, что вердикт считается по первому числу');
  assert.match(head.textContent, /Второе — как считает карта/,
    'шапка не говорит, что второе число — то же, что на карте тела');
});
...
// Финальное ревью (В5), шестой беззубый тест этой работы: «свёрнутое состояние
// тоже помнится» проверяет запись '0' в хранилище — но не то, что панель
// действительно скрылась. Мутация app.js `weeklyBody.hidden = !open` →
// `= false` (свернуть больше нельзя) оставляла все тесты зелёными: покрыта
// была ровно одна половина поведения. У секции-образца — карты тела — эта же
// мутация ловится тестом «повторный клик сворачивает» (test/
// bodymap-section.test.js), устройство секции скопировали, а тест нет.
test('повторный клик сворачивает секцию', () => {
  const { window, document } = mount();
  startApp(window);
  ...
  assert.equal(document.querySelector('.weekly-body').hidden, true,
    'после второго клика панель обязана быть скрыта, а не только помечена свёрнутой');
});
