...
Для каждой мутации: применить, прогнать `node --test`, записать результат, откатить, проверить `git status --porcelain` пустым.

**Как откатывать новый файл.** `git checkout -- weekly.js` на файле, который ни разу не попадал в индекс, не работает: git не знает, к чему возвращать. Поэтому **сначала выполни `git add weekly.js test/weekly.test.js`** (первую команду шага 8), и только потом мутируй — тогда `git checkout --` откатывает к добавленному. Проверяй откат и через `git diff`, а не только через статус.
...
Откатывать новый файл — так же, как в Task 1: `git add load-bands.js test/load-bands.test.js` **до** первой мутации, иначе `git checkout --` не к чему возвращать.
