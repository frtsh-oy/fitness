...
export function startApp(win = globalThis.window) {
  ...
  function fillNotes() {
    ...
    // это не край случая, а другая тренировка (например, такая, где у всех
    // групп разминки есть и силовые подходы). Через реестр эта ветвь
    // недостижима — в нём одна тренировка, и оба списка у неё непусты, —
    // поэтому тестами она не покрыта: мутацией показано, что без проверки
    // length тесты остаются зелёными. Без неё в подписи оставалось бы
    // «Только в разминке и заминке: .».
    const parts = [];
    // Про разминку и заминку речь только в силовом режиме: там эти группы
    // бледные, и объяснить их бледность больше нечем. В режиме всей нагрузки
    // они раскрашены наравне с силовыми, и говорить о них нечего.
