...
export function createBodyMap({ workout, anteriorHost, posteriorHost, onPick, mode = DEFAULT_MODE }) {
  ...
  const dataOf = (rows, chosen) => {
    ...
    // Выбранной области может не быть в сводке текущего режима: например, у
    // задней дельты нет силовых подходов — её единственное упражнение
    // разминочное. Заливать область всё равно надо, поэтому запись для неё
  ...
  const handle = ({ muscle }) => {
    ...
    // обычный случай (например, задняя дельта в силовом режиме), а не край:
    // ноль подходов и пустой список.
